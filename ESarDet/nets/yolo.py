...
import math
from .darknet import BaseConv, CSPDarknet, CSPLayer, DWConv, dilateConv, DDWConv
...
class ECCSPLayer(nn.Module):
    def __init__(self, in_channels, out_channels, n=1, shortcut=True, expansion=0.5, depthwise=False,act="silu",convsize=1,dia=1):
        ...
        #   大的残差边部分的初次卷积
        # --------------------------------------------------#
        self.conv2 = DDWConv(in_channels, hidden_channels,convsize, stride=1, act=act,dia=dia)
        # -----------------------------------------------#
        #   对堆叠的结果进行卷积的处理
        # -----------------------------------------------#
